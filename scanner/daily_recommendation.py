...
from engine.opportunity_ranking_engine import OpportunityRankingEngine
...
class DailyRecommendation:
    ...
    def get_technical_candidates(self):
        ...
        most_active_cache.parent.mkdir(
            parents=True,
            exist_ok=True
        )
        ...
        print()
        print(
            f"FAST SCREENER TIME  : "
            f"{fast_elapsed:.2f} DETIK"
        )
        ...
        early_trend_candidates = [
            stock
            for stock in fast_results
            if (
                stock.get("early_trend") is True
                and stock.get(
                    "liquidity_status",
                    "ILLIQUID"
                ) in [
        ...
        early_trend_codes = {
            stock["code"]
            for stock in early_trend_candidates
        }

        # Tambahkan Early Trend tanpa menghapus
        # kandidat Fast Screener lama
        candidate_map = {
            stock["code"]: stock
        ...
        technical_elapsed = time.time() - technical_start

        print()
        print(
            f"TECHNICAL TIME      : "
            f"{technical_elapsed:.2f} DETIK"
        )
        ...
        early_trend_ranked_candidates = [
            stock
            for stock in ranked_results
            if stock["code"] in early_trend_codes
        ]

        for stock in early_trend_ranked_candidates:
            stock["early_trend"] = True

        early_trend_ranked_candidates.sort(
            key=lambda x:
            x["ranking_score"],
            reverse=True
        )
        ...
        early_trend_ranked_candidates = (
            early_trend_ranked_candidates[:15]
        )
        ...
        print(
            "Early Trend        :",
            len(early_trend_ranked_candidates)
        )
    ...
    def analyze_candidate(stock):
        ...
        try:
            ...
            mtf = MultiTimeFrameEngine.analyze(
                code
            )
    ...
    def analyze_single_stock(self, code):
        ...
        # Fast Screener hanya 1 saham
        fast = FastScreener(
            stocks=[code],
            period=self.period
        )

        fast_results = fast.scan()

        if not fast_results:
            return None

        # Technical Screener hanya 1 saham
        technical = TechnicalScreener(
            candidates=fast_results,
            period=self.period
        )

        technical_results = technical.scan()

        if not technical_results:
            return None

        stock = technical_results[0]

        # Ranking
        ranking = TechnicalRankingEngine.calculate(stock)
        stock.update(ranking)

        # =====================================================
        # DEEP ANALYSIS
        # =====================================================

        stock = self.analyze_candidate(stock)
        ...
        watchlist_result = (
            WatchlistPriorityEngine.calculate(
        ...
        opportunity_result = (
            OpportunityRankingEngine.calculate(

                recommendation=stock.get(
                    "recommendation",
                    "WAIT"
                ),
                ...
                watchlist_priority_score=stock.get(
...
if __name__ == "__main__":
    ...
    engine.print_opportunity_ranking(
        results,
        limit=10
    )
